    ...
    static <ResponseT> ResponseTransformer<ResponseT, ResponseT> toFile(Path path) {
        return (resp, in) -> {
            try {
                ...
                // Retry the request
                throw RetryableException.builder().message(copyError).throwable(copyException).build();
            }
        };
    }
    ...
    static <ResponseT> ResponseTransformer<ResponseT, ResponseBytes<ResponseT>> toBytes() {
        return (response, inputStream) -> {
            try {
                ...
                throw RetryableException.builder().message("Failed to read response.").throwable(e).build();
            }
        };
    }
